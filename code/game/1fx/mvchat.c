// MVChat system as-is from 1fxmod.
...
void mvchat_parseFiles()
{
    ...
    for (i = 0; i < mvchatCount; i++) {
        ...
        Com_Memset(mvchatFiles[i], 0, MAX_QPATH * sizeof(char));
    }
    ...
    for (i = 0; i < mvchatCount; i++) {
        ...
        mvchatFile = trap_GP_ParseFile(va("files/mvchats/%s", mvchatFiles[i]));
        Com_Printf("... %s\n", mvchatFiles[i]);
        if (!mvchatFile) {
        ...
        while (mvchatGroup) {
            ...
            char                isCustomSound[2];

            // Get the base information for this sound.
            // Store a copy of the GP2 group name.
            trap_GPG_GetName(mvchatGroup, mvchatGroupName, sizeof(mvchatGroupName));

            // Get the sound number and text.
            trap_GPG_FindPairValue(mvchatGroup, "number", "0", sNumText, sizeof(sNumText));
            trap_GPG_FindPairValue(mvchatGroup, "text", "", sText, sizeof(sText));
            ...
            Com_Memset(mvchatSound->sounds, 0, sizeof(mvchatSoundLang_t) * MVCHAT_NUM_LANGS);
            ...
            // check whether the sound we're parsing is a customsound.
            // will not add a static hardcoded list nor a dirtree parser to see whether it is in a default pk3 (or RPM pk3) or not.
            // so the functionality will rely on having it defined as a customsound in the mvchat file itself.
            trap_GPG_FindPairValue(mvchatGroup, "customsound", "0", isCustomSound, sizeof(isCustomSound));
            mvchatSound->isCustomSound = atoi(isCustomSound);
            ...
            for (x = 0; x < MVCHAT_NUM_LANGS; x++) {
                ...
                if (langSoundGender) {
                    ...
                    Com_Memset(sSound, 0, sizeof(sSound));
                    trap_GPG_FindPairValue(langSoundGender, "sound", "", sSound, sizeof(sSound));
                    if (sSound[0] != 0) {
                ...
                Com_Memset(sOption, 0, sizeof(sOption));
                trap_GPG_FindPairValue(langSound, "default", "no", sOption, sizeof(sOption));
                ...
                Com_Memset(sOption, 0, sizeof(sOption));
                trap_GPG_FindPairValue(langSound, "next", "no", sOption, sizeof(sOption));
                if (Q_stricmp(sOption, "yes") == 0) {
...
void mvchat_chatDetermineSound(mvchat_ChatParse_t* chatParse, char* chatText, TIdentity* identity)
{
    ...
    chatParse->isCustomSound = mvchatSounds[num]->isCustomSound;
}
...
void mvchat_findSounds(gentity_t* ent) {
    int             i, x;
    char            buf2[1000] = { 0 };
    char            filter[MAX_STRING_TOKENS];
    qboolean        soundListed;
    mvchatSound_t* mvsound;

    if (trap_Argc() < 2) {
        G_printInfoMessage(ent, "Usage: /findsound soundtext");
        return;
    }
    ...
    if (strlen(filter) < 3) {
        G_printInfoMessage(ent, "Please enter at least 3 characters to run the search.");
        return;
    }
    ...
    for (i = 0; i < MVCHAT_MAX_SOUNDS; i++) {
        ...
        if (strlen(buf2) > 820) {
            ...
            Com_Memset(buf2, 0, sizeof(buf2));
        }

        if (Q_stristr(mvsound->textClean, filter)) {
