...
void RPM_UpdateTMI(void)
{
    ...
    for (i = 0; i < level.numConnectedClients; i++)
    {
        ...
        bestLoc = Team_GetLocation(&g_entities[level.sortedClients[i]], qfalse);
        ...
        if (g_entities[level.sortedClients[i]].client->sess.clientMod == CL_RPM && atof(g_entities[level.sortedClients[i]].client->sess.clientVersion) > 0.7) {
