...
void SP_accelerator_touch(gentity_t* self, gentity_t* other, trace_t* trace) {
        ...
        if (other->client->sess.team != self->team2) {
            if (level.time >= other->client->sess.lastmsg) {
                if (self->team2 == TEAM_RED) {
                    ...
                    G_Broadcast(BROADCAST_GAME, other, qfalse, "Accelerator is for %s only!", g_customBlueName.string);
                }
    ...
    G_CloseSound(other->r.currentOrigin, sound2);
    // Boe!Man 5/22/12: End.
        ...
        if (other->client->sess.team != self->team2) {
            if (level.time >= other->client->sess.lastmsg) {
                if (self->team2 == TEAM_RED) {
                    ...
                    G_Broadcast(BROADCAST_GAME, other, qfalse, "Teleporter is for %s only!", g_customBlueName.string);
                }
...
void trigger_ReachableObject_touch(gentity_t* self, gentity_t* other, trace_t* trace)
{
    ...
    if (self->team2 != TEAM_FREE) {
        if (other->client->sess.team != self->team2) {
            if (level.time >= other->client->sess.lastmsg) {
                if (self->team2 == TEAM_RED) {
                    G_Broadcast(BROADCAST_GAME, other, qfalse, "Object is for %s only!", g_customRedName.string);
                }
                else {
                    G_Broadcast(BROADCAST_GAME, other, qfalse, "Object is for %s only!", g_customBlueName.string);
                }
    ...
    if (self->score > 0) {
        ...
        if (level.gametypeData->teams) { // JANFIXME Don't add H&S/H&Z points?
        ...
        if (other->client->sess.team != self->team2) {
            if (level.time >= other->client->sess.lastmsg) {
                if (self->team2 == TEAM_RED) {
                    ...
                    G_Broadcast(BROADCAST_GAME, other, qfalse, "Booster is for %s only!", g_customBlueName.string);
                }
