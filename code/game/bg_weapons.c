...
static qboolean BG_ParseWeapon(weapon_t weapon, void *group)
{
	...
	while(sub)
	{
		...
		else if (Q_stricmp(name, "surfaces") == 0)
		{
			...
			{
				trap_GPG_GetName(surfaceCallbackName, weaponParseInfo[weapon].mSurfaceCallbacks[i].mName, sizeof(weaponParseInfo[weapon].mSurfaceCallbacks[i].mName));
				surfaceCallbackValue = trap_GPG_GetPairs(surfaceCallbackName);
				for(j=0;surfaceCallbackValue && (j<MAX_CALLBACK_SURFACES);j++)
				{
					trap_GPG_GetName(surfaceCallbackValue, weaponParseInfo[weapon].mSurfaceCallbacks[i].mOnOffSurfaces[j].mName, sizeof(weaponParseInfo[weapon].mSurfaceCallbacks[i].mOnOffSurfaces[j].mName));
