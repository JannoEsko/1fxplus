...
vmCvar_t            gt_humanTeamColored;
vmCvar_t            gt_zombieTeamColored;

static cvarTable_t gametypeCvarTable[] =
{
    { &gt_simpleScoring,    "gt_simpleScoring",     "0",  CVAR_ARCHIVE, 0.0f, 0.0f, 0, qfalse },
    { &gt_humanTeamColored,      "gt_humanTeamColored",   "^yZ^lo^lm^+b^7ies", 0.0f, 0.0f, 0, qfalse },
    { &gt_zombieTeamColored, "gt_zombieTeamColored", "^1H^Tu^om^qa^+n^7s", CVAR_ARCHIVE, 0.0, 0.0, 0,  qfalse },
...
int GT_Event(int cmd, int time, int arg0, int arg1, int arg2, int arg3, int arg4)
{
    ...
    case GTEV_TEAM_ELIMINATED:
        ...
        {
        case TEAM_RED:
            trap_Cmd_TextMessage(-1, va("%s team \\eliminated!", gt_humanTeamColored.string));
            trap_Cmd_AddTeamScore(TEAM_BLUE, 1);
            trap_Cmd_Restart(5);
            break;

        case TEAM_BLUE:
            trap_Cmd_TextMessage(-1, va("%s team \\eliminated!", gt_zombieTeamColored.string));
    ...
    case GTEV_TIME_EXPIRED:
        trap_Cmd_TextMessage(-1, va("%s have \\won the round!", gt_humanTeamColored.string));
        trap_Cmd_AddTeamScore(TEAM_RED, 1);
        trap_Cmd_Restart(5);
        break;
